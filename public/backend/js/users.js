    ...
    sv.data.changeStatusUser(url, data, function (response) {
        if (response.status == "success") {
            if (response.code == 1) {
                ...
                var html = '';
                html += '<button type="button" class="btn btn-round btn-danger btn-xs btnChangeStatus' + id + '" onclick="btnChangeStatus(' + id + ')">No Active</button>';
                $('.btnChangeStatus' + id).replaceWith(html);
            }
        } else {
...
$('.btnDelete').click(function () {
    ...
    var id = $(this).parents('tr').find('input[type=checkbox]').attr('data-id');
    var arrid = [];
